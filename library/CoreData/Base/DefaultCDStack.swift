    ...
    */
    internal func addDatabase(completionClosure: CompletionClosure)
    {
        ...
        if (error?.domain == NSCocoaErrorDomain as String) && isMigratingError {
            ...
            self.persistentStoreCoordinator!.lock()
            do {
                try self.persistentStoreCoordinator!.addPersistentStoreWithType(NSSQLiteStoreType, configuration: nil, URL: self.databasePath, options: DefaultCDStack.defaultStoreOptions())
            } catch let error1 as NSError {
                error = error1
            }
...
public extension NSManagedObjectContext
{
    ...
    */
    func contextWillSave(notification: NSNotification) {
        ...
        let saved: Bool
        do {
            let all = insertedObjects.allObjects as! [NSManagedObject]
            try context.obtainPermanentIDsForObjects(all)
            saved = true
        } catch let error1 as NSError {
            error = error1
            saved = false
